...
namespace ego_planner
{
  ...
  private:
    ...
    ros::Publisher central_points_pub;
    ros::Publisher central_init_list_pub;
    ros::Publisher optimal_list_pub;
    ros::Publisher failed_list_pub;
    ros::Publisher central_list_pub;
  ...
  public:
    ...
    void displayECBSpoints(std::vector<Eigen::Vector3d> points, Eigen::Vector4d color, int id);
    void displayCentralInitPathList(std::vector<Eigen::MatrixXd> cstr_ptss, const double scale);
    void displayOptimalList(Eigen::MatrixXd optimal_pts, int id);
    void displayFailedList(Eigen::MatrixXd failed_pts, int id);
    void displayCentralList(std::vector<Eigen::MatrixXd> cen_pts, int id);
