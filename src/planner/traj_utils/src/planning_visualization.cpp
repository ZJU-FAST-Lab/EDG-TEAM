...
namespace ego_planner
{
  PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh)
  {
    ...
    central_points_pub = nh.advertise<visualization_msgs::Marker>("ecbs_search_points", 2);
    central_init_list_pub = nh.advertise<visualization_msgs::Marker>("central_init_list", 2);
    ...
    central_list_pub = nh.advertise<visualization_msgs::Marker>("central_list", 2);
  ...
  void PlanningVisualization::displayCentralInitPathList(std::vector<Eigen::MatrixXd> cstr_ptss, const double scale)
  {
    if (central_init_list_pub.getNumSubscribers() == 0)
    {
      return;
    }

    for ( int id=0; id<(int)cstr_ptss.size(); id++ )
    {
      vector<Eigen::Vector3d> list;
      for(int j = 0; j < cstr_ptss[id].cols(); j++)
      {
        Eigen::Vector3d pt = cstr_ptss[id].col(j).transpose();
        list.push_back(pt);
      }
      Eigen::Vector4d color(0, 0, 1, 0.7);
      displayMarkerList(central_init_list_pub, list, scale, color, id, false);
      ros::Duration(0.001).sleep();
    }
  ...
  void PlanningVisualization::displayCentralList(std::vector<Eigen::MatrixXd> cen_pts, int id)
  {

    if (central_list_pub.getNumSubscribers() == 0)
    {
      return;
    }

    for(int i = 0; i < (int)cen_pts.size(); i++)
    {
      vector<Eigen::Vector3d> list;
      for (int j = 0; j < cen_pts[i].cols(); j++)
      {
        Eigen::Vector3d pt = cen_pts[i].col(j).transpose();
        list.push_back(pt);
      }
      Eigen::Vector4d color(0, 1, 0, 1);
      displayMarkerList(central_list_pub, list, 0.15, color, i);
      ros::Duration(0.001).sleep();
    }

  }

  void PlanningVisualization::displayECBSpoints(std::vector<Eigen::Vector3d> points, Eigen::Vector4d color, int id)
  {
    if (central_points_pub.getNumSubscribers() == 0)
    {
      return;
    }
    ...
    central_points_pub.publish(sphere);
