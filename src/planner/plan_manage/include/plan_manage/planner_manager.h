...
#include <path_searching/environment.hpp>
...
namespace ego_planner
{
  ...
  public:
    ...
    bool ecbsSearch(std::vector<PlanResult<State, Action, double>> &solution,
      const std::vector<Eigen::Vector3d> &start_pts,
      const std::vector<Eigen::Vector3d> &local_target_pts);

    bool EmergencyStop(Eigen::Vector3d stop_pos);
    bool checkCollision(int drone_id);
    bool setLocalTrajFromOpt(const poly_traj::MinJerkOpt &opt, const bool touch_goal, const double &world_time);
    ...
    TrajContainer traj_;
    EcbsParameters ep_;
    std::vector<poly_traj::MinJerkOpt> best_MJOs_;
