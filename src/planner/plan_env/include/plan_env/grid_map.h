...
struct MappingParameters
{
  ...
  /* ECBS search share map */
  Eigen::Vector3i local_map_voxel_num_;
...
struct MappingData
{
  ...
  std::vector<char> local_buffer_;//share obstacle for central optimization;
...
public:
  ...
  /* ECBS local map toAddress */
  inline int toLocAddress(int x, int y, int z);
...
inline int GridMap::toLocAddress(int x, int y, int z)
{
  return x * mp_.local_map_voxel_num_(1) * mp_.local_map_voxel_num_(2) + y * mp_.local_map_voxel_num_(2) + z;
}
