...
ros::Subscriber other_odoms_sub_, one_traj_sub_, central_state_sub_, cen_replan_sub_, mandatory_stop_sub_, goal_sub_;
ros::Publisher other_odoms_pub_, one_traj_pub_, central_state_pub_, cen_replan_pub_, mandatory_stop_pub_, goal_pub_;
string udp_ip_;
int drone_id_;
...
traj_utils::CentralState CentralState_msg_;
std_msgs::UInt8MultiArray group_msg_;
...
void central_state_sub_udp_cb(const traj_utils::CentralStatePtr &msg)
{

  int len = serializeTopic(MESSAGE_TYPE::CENTRAL_STATE, *msg);
...
void cen_replan_sub_udp_cb(const std_msgs::UInt8MultiArrayPtr &msg)
{

  int len = serializeTopic(MESSAGE_TYPE::CEN_REPLAN, *msg);
...
void udp_recv_fun()
{
  ...
  while (true)
  {
    ...
    case MESSAGE_TYPE::CENTRAL_STATE:
    {

      if (valread == deserializeTopic(CentralState_msg_))
      {
        central_state_pub_.publish(CentralState_msg_);
      }
      else
      {
    ...
    case MESSAGE_TYPE::CEN_REPLAN:
    {

      if (valread == deserializeTopic(group_msg_))
      {
        cen_replan_pub_.publish(group_msg_);
      }
      else
      {
    ...
    case MESSAGE_TYPE::GOAL:
    {
      ...
      {
        ROS_ERROR("Received message length not matches the sent one (6)!!!");
        continue;
      }
...
int main(int argc, char **argv)
{
  ...
  central_state_sub_ = nh.subscribe("/local_target_from_planner", 10, central_state_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  central_state_pub_ = nh.advertise<traj_utils::CentralState>("/local_target_to_planner", 10);

  cen_replan_sub_ = nh.subscribe("/cen_replan_from_planner", 10, cen_replan_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  cen_replan_pub_ = nh.advertise<std_msgs::UInt8MultiArray>("/cen_replan_to_planner", 10);
