...
namespace ego_planner
{
  ...
  public:
    ...
    bool ecbsOptTraj(const std::vector<Eigen::Matrix<double, 3, 3>> &iniStates, const std::vector<Eigen::Matrix<double, 3, 3>> &finStates,
    ...
    CHK_RET finelyCheckAndSetConstraintPoints(ConstraintPoints &cps, std::vector<std::pair<int, int>> &segments,
    ...
    double computePerturedSmoothCost(Eigen::VectorXd &x_var, std::vector<poly_traj::MinJerkOpt> &jerkOpts_pert);
    // double computePerturedCost(Eigen::VectorXd &x_var, std::vector<poly_traj::MinJerkOpt> &jerkOpts_pert, std::vector<ConstraintPoints> &cpss_pert);
  ...
  private:
    ...
    template <typename EIGENVEC>
    void addPVAGradCost2CT(poly_traj::MinJerkOpt &jerkOpt, ConstraintPoints &cps, EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K);

    template <typename EIGENVEC>
    void cenAddPVAGradCost2CT(int id, std::vector<Eigen::VectorXd> &Ts, 
                            ConstraintPoints &cps, std::vector<EIGENVEC> &gradTs, Eigen::VectorXd &costs, const int &K);
