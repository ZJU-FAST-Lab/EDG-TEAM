...
class ECBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    // the conflict number of CT's start node
    ...
    // update focal list
...
#else
      {
        ...
        bestCost = open.top().cost; // min cost in OPEN list 
        if (bestCost > oldBestCost) {
          auto iter = open.ordered_begin();
          auto iterEnd = open.ordered_end();
          for (; iter != iterEnd; ++iter) {
            Cost val = iter->cost;
            // Expand the FOCAL list
...
// check focal list/open list consistency
      ...
      focal.pop(); // pop min conflict num of node
      open.erase(h);
      ...
      if (!m_env.getFirstConflict(P.solution, conflict)) {
        if(log) {
            ...
            std::cout << "highLevelExpanded nums: " << m_env.highLevelExpanded() << std::endl;
        }
        solution = P.solution;
        return true;
      }
      ...
      // convert constraint to a child node, add the node to OPEN list
...
// boost::heap::compare<compareFocalHeuristic> > focalSet_t;
  ...
  typedef typename boost::heap::d_ary_heap<
      ...
      focalSet_t; // focalSet_t: use compareFocalHeuristic to order
#endif
