#ifndef EDG_SWARM_ENVIRONMENT_H
#define EDG_SWARM_ENVIRONMENT_H

#include <path_searching/ecbs.hpp>
...
namespace libMultiRobotPlanning {
    struct State {
        State(double time, double x, double y, double z) : time(time), x(x), y(y), z(z) {}
...
namespace libMultiRobotPlanning {
    ...
    public:
        ...
        void setLowLevelContext(size_t agentIdx, const Constraints *constraints) {
            ...
            m_lastGoalConstraint = -1; // the time of  last goal constraint
        ...
        // add heuristic function t
        double admissibleHeuristic(const State &s) {
            double d = computeDistance(s);

            return std::abs(s.x - m_goals[m_agentIdx].x) +
                   std::abs(s.y - m_goals[m_agentIdx].y) +
                   std::abs(s.z - m_goals[m_agentIdx].z) + 0.000001 * d; //must smaller than 1e-6, otherwise not mean.
        }

        // implement tie breaker  distance=norm(cross(v1-v2,pt-v2))/norm(v1-v2)
        double computeDistance(const State &s) {

            Eigen::Vector3d v1(m_starts[m_agentIdx].x, m_starts[m_agentIdx].y, m_starts[m_agentIdx].z);
            Eigen::Vector3d v2(m_goals[m_agentIdx].x, m_goals[m_agentIdx].y, m_goals[m_agentIdx].z);

            Eigen::Vector3d pt(s.x, s.y, s.z);
            Eigen::Vector3d v12 = v1 - v2;
            Eigen::Vector3d pv2 = pt - v2;
            double distance = (v12.cross(pv2)).norm() / v12.norm();

            return distance;
        }

        // low-level, get numConflict(equal state) from given solution; distance-to-go
            ...
            neighbors.clear();
            {
                ...
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Wait, 1));
                }
            }
            {
                ...
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Left, 1));
                }
            }
            {
                State n(s.time + 1, s.x + 1, s.y, s.z);
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Right, 1));
                }
            }
            {
                ...
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Up, 1));
                }
            }
            {
                State n(s.time + 1, s.x, s.y - 1, s.z);
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Down, 1));
                }
            }
            {
                ...
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Top, 1));
                }
            }
            {
                State n(s.time + 1, s.x, s.y, s.z - 1);
                if (stateValid(n) && transitionValid(s, n)) {
                    neighbors.emplace_back(
                            Neighbor<State, Action, double>(n, Action::Bottom, 1));
                }
            }
    ...
    private:
        ...
        bool stateValid(const State &s) {
            ...
            Eigen::Vector3d pos;
            m_grid_map->indexToPos(Eigen::Vector3i(s.x, s.y, s.z), pos);

            // O(1) time complexity to determine whether the traversed node is occupied
            return !m_grid_map->getInflateOccupancy(pos) &&
    ...
    private:
        ...
        double m_grid_size;
        GridMap::Ptr m_grid_map;
    };
}
#endif //EDG_SWARM_ENVIRONMENT_H
