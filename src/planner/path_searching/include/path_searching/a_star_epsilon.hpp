...
algorithm that finds the shortest path for a given map up to a suboptimality factor.
It uses an admissible heuristic (to keep track of the optimum) and an
inadmissible heuristic (to guide the search within a suboptimal bound w.)
...
class AStarEpsilon {
 public:
  ...
  bool search(const State& startState,
    ...
    neighbors.reserve(10); // set number > max neighbors number
      ...
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
        if (closedSet.find(neighbor.state) == closedSet.end()) {
          ...
          if (iter == stateToHeap.end()) {  // Discover a new node
            ...
            // We found this node that can get a better path.
