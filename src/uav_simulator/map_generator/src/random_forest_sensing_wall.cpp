...
// std::vector<double> initPos, waypoint0Pos, waypoint1Pos, targetPos;
...
void RandomMapGenerateCylinder()
{
  ...
  // large scale swarm simulation
  // avoid drones initPos and target
  // int size = initPos.size() / 2;
  // for(int i = 0; i < size; i++)
  // {
  //   obs_position.push_back(Eigen::Vector2d(initPos[2*i], initPos[2*i+1]));
  //   obs_position.push_back(Eigen::Vector2d(waypoint0Pos[2*i], waypoint0Pos[2*i+1]));
  //   obs_position.push_back(Eigen::Vector2d(waypoint1Pos[2*i], waypoint1Pos[2*i+1]));
  //   obs_position.push_back(Eigen::Vector2d(targetPos[2*i], targetPos[2*i+1]));
  // }
...
{
  pcl::PointXYZ pt_random;
  pcl::PointCloud<pcl::PointXYZ>::iterator idx;

  for (double x = -6.0; x <= 6.0; x += 0.1)
    for (double y = -0.5; y <= 0.5; y += 0.1)
      for (double z = -0.5; z <= 4.0; z += 0.1)
      {
        if(((x >= -0.40) & (x <= 0.40)) & ((y >= -0.5) & (y <= 0.5)) & ((z >= 1.0) & (z <= 2.5)))
          continue;
  

        // two square
        // if(((x >= -1.5) & (x <= -0.5)) & ((y >= -0.5) & (y <= 0.5)) & ((z >= 1.0) & (z <= 2.0)))
        //   continue;

        // if(((x >= 0.5) & (x <= 1.5)) & ((y >= -0.5) & (y <= 0.5)) & ((z >= 1.0) & (z <= 2.0)))
        //   continue;
...
{
  ros::init(argc, argv, "random_map_sensing_wall");
  ros::NodeHandle n("~");
  ...
  // large scale swarm simulation
  // n.param<std::vector<double>>("position/initPos", initPos, std::vector<double>());   
  // n.param<std::vector<double>>("position/waypoint0Pos", waypoint0Pos, std::vector<double>());  
  // n.param<std::vector<double>>("position/waypoint1Pos", waypoint1Pos, std::vector<double>());   
  // n.param<std::vector<double>>("position/targetPos", targetPos, std::vector<double>());    
